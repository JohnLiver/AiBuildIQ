...
const Success = () => {
  ...
  const download_pdf = async () => {
    ...
    try {
      const backend = `http://${process.env.BACKEND_HOST || 'localhost'}:${process.env.BACKEND_PORT || '8000'}`;
      const endpoint = `/api/loan-request/download`
      const response = await fetch(backend+endpoint, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(fundingRequestId),
      })
      ...
      if (response.ok) {
        ...
        // if (contentType && contentType.includes('application/pdf')) {
          ...
          link.download = "generated-file.pdf" // Specify the filename
